...
public class sr_sender implements ActionListener{
	...
	static HashMap<Integer,Integer> map=new HashMap<Integer,Integer>();// to map sequenceNo of pkt,wth its allocated timmer index
	 	...
	 	if(q.size()>0){
	 		...
	 		timerr[timerIndexLeft]=new Timer(300,new ActionListener(){
	 			public void actionPerformed(ActionEvent e3){ 
	 				...
	 				Duration runningTime=Duration.between(startTime[timerIndexLeft],now);//System.out.println("2");
					...
					timeDisplay[pkttNo].setText(String.format("00m %02ds", timeLeft.toSeconds()));//System.out.println("4");
 				}
 			});
 			startTime[timerIndexLeft]=LocalDateTime.now();//System.out.println("5");
 			timerr[timerIndexLeft].start();//System.out.println("6");
 			map.put(pkttNo,timerIndexLeft);//dlm.addElement("map:"+map.toString());dlm.addElement("q:"+q);
	...
	public static boolean sendNewPressed(boolean pktKilled){
		if(nextSeqNo<base+5){
			...
			timeDisplay[nextSeqNo].setVisible(true);/*enabling JLabel to show timer readings,for this packet */
		...
		if(t.size()>0){
			return true;}
		else{return false;}// checking if any timer got timed out,while user was selecting (to kill)(dont kill) new packet
	...
	public static void updateBase(){
		...
		for(y=b_old;y<base;y++){
			...
			if(!sendNew.isEnabled() &&(nextSeqNo-base<5)){
				sendNew.setEnabled(true);}// as only 5 packets,are allowed to be sent,without receiving ack,so sendnew button was disabled
	...
	public static void main(String[] args)throws Exception {
		...
		q.add(3);q.add(4);
		//dlm.addElement("map(initial):"+map.toString());dlm.addElement("q(initial):"+q);
		//receving ack
		int w=0;
		while(true){ 
			try{
				...
				if(t.contains(w)){
					...
					if(map.containsKey(w)){
						...
						q.add(map.remove(w));
						// dlm.addElement("map:"+map.toString());
						//dlm.addElement("q:"+q); 
						// dlm.addElement("t:"+t); 
